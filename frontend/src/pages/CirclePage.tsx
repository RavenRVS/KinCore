...
const CirclePage: React.FC = () => {
  ...
  const isAdmin = currentLevel.type === 'circle' && currentLevel.isAdmin;

  // Получить текущий код (после генерации)
  const fetchCredentials = async () => {
    if (currentLevel.type !== 'circle' || !currentLevel.id) return;
    setLoading(true);
    setError('');
    setSuccess('');
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`http://localhost:8000/api/famcircle/circles/`, {
        headers: { 'Authorization': `Token ${token}` }
      });
      const circles = await res.json();
      const circle = circles.find((c: any) => c.id === currentLevel.id);
      setCredentials({ join_code: circle?.join_code || '' });
    } catch {
      setCredentials(null);
    } finally {
      setLoading(false);
  ...
  const handleRegenerate = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`http://localhost:8000/api/famcircle/circles/${currentLevel.id}/regenerate_credentials/`, {
        method: 'POST',
        headers: { 'Authorization': `Token ${token}` }
      });
      if (!res.ok) {
  ...
  return (
    <div className="circle-page">
      <main>
        <h1>Главная страница семейного круга</h1>
        <p>ID круга: {id}</p>
        <div style={{ maxWidth: 600, margin: '0 auto', padding: 24 }}>
          <p>Здесь будет содержимое главной страницы круга.</p>

          {/* Секция для админа */}
          {isAdmin && (
            <div style={{ marginTop: 32, padding: 20, background: '#f8f9fa', borderRadius: 8, boxShadow: '0 1px 4px #0001' }}>
              <h3>Пригласить в круг</h3>
